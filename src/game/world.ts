...
export function buildWorld(): CircuitWorld {
	...
	for (let i = 0; i < 12; i++) {
		...
		const vein = new Mesh(new BoxGeometry(6.4, .22, 90), i % 2 ? matRiver : matRiverGold);
		vein.position.set(Math.cos(a) * 70, 1.42, Math.sin(a) * 70);
		vein.rotation.y = a;
		group.add(vein);
	}
	...
	const matInnerCore = new MeshPhysicalMaterial({
		color: 13172728,
		emissive: 1747144,
		emissiveIntensity: CORE_ONLY ? .7 : .92,
		roughness: CORE_ONLY ? .1 : .04,
		metalness: .08,
		transmission: CORE_ONLY ? .38 : .55,
		thickness: CORE_ONLY ? 1.6 : 3.4,
		ior: 1.5,
		iridescence: CORE_ONLY ? .8 : 1,
		iridescenceIOR: 1.32,
		...
		transparent: true,
		opacity: .94,
		envMapIntensity: CORE_ONLY ? 1.35 : 1.75,
		sheen: .72,
		sheenColor: new Color(8320767),
		attenuationColor: new Color(4903144),
		attenuationDistance: CORE_ONLY ? 2.2 : 3.6
	});
	let innerCore;
	if (CORE_ONLY) {
		const pad = new Mesh(new CylinderGeometry(5.2, 6.4, 1.1, 8), matSlab);
		pad.position.y = .7;
		pad.receiveShadow = true;
		spire.add(pad);
		const lip = new Mesh(new TorusGeometry(5.4, .18, 6, 16), matGoldSoft);
		lip.rotation.x = Math.PI / 2;
		lip.position.y = 1.24;
		spire.add(lip);
		const shaft = new Mesh(new CylinderGeometry(1.02, 2.05, 11.2, 6), matSpire);
		shaft.position.y = 6.85;
		spire.add(shaft);
		const belt = new Mesh(new TorusGeometry(1.48, .12, 6, 12), matGlow);
		belt.rotation.x = Math.PI / 2;
		belt.position.y = 5.2;
		spire.add(belt);
		innerCore = new Mesh(new OctahedronGeometry(1.12, 0), matInnerCore);
		innerCore.scale.set(.62, 1.32, .62);
		innerCore.position.y = 12.05;
		innerCore.userData.baseY = 1.32;
		spire.add(innerCore);
		const peak = new Mesh(new ConeGeometry(1.08, 3.2, 6), matHeart);
		peak.position.y = 14.7;
		spire.add(peak);
		const shaftGlow = new Mesh(new CylinderGeometry(2.05, 3.4, 15, 8, 1, true), new MeshBasicMaterial({
			color: 4903136,
			transparent: true,
			opacity: .08,
			blending: 2,
		...
		shaftGlow.position.y = 8.2;
		spire.add(shaftGlow);
		for (let i = 0; i < 4; i++) {
			const a = i / 4 * Math.PI * 2 + .4;
			const shard = new Mesh(new OctahedronGeometry(.48, 0), i % 2 ? matCrystal : matHeart);
			shard.position.set(Math.cos(a) * 4.05, 1.45, Math.sin(a) * 4.05);
			shard.rotation.set(.32, a, .18);
			shard.scale.set(.42, 1.2 + i * .12, .42);
			spire.add(shard);
		}
	} else {
	[
	...
	spire.add(shaft);
	innerCore = new Mesh(new OctahedronGeometry(5.2, 1), matInnerCore);
	innerCore.scale.set(.55, 2.6, .55);
	innerCore.position.y = 148;
	innerCore.userData.baseY = 2.6;
	spire.add(innerCore);
	...
	group.add(spire);
	group.add(new PointLight(3073791, CORE_ONLY ? 3.2 : 9, CORE_ONLY ? 90 : 620, 1.35));
	const hubLight = new PointLight(8313070, CORE_ONLY ? 2.4 : 6.5, CORE_ONLY ? 72 : 440, 1.3);
	hubLight.position.set(0, CORE_ONLY ? 16 : 210, 0);
	group.add(hubLight);
	const goldKiss = new PointLight(14725216, CORE_ONLY ? 1.6 : 3.6, CORE_ONLY ? 48 : 280, 1.5);
	goldKiss.position.set(CORE_ONLY ? 7 : 50, CORE_ONLY ? 8 : 64, CORE_ONLY ? 4 : 36);
	group.add(goldKiss);
	...
	if (!CORE_ONLY) DISTRICTS.forEach((D) => {
		laterOn(() => {
			const zg = new Group();
	...
	if (!CORE_ONLY) laterOn(() => stamp(geoDen, matSpire, 150, 340, Math.ceil(48), 8, 14, 11, 7, 11));
	if (!CORE_ONLY) laterOn(() => stamp(geoHallBlock, matCyan, 200, 520, Math.ceil(28), 18, 32, 17, 9, 14));
	if (!CORE_ONLY) laterOn(() => stamp(geoNeedle, matGold, 280, 640, Math.ceil(36), 22, 48, 23, 5, 8));
	if (!CORE_ONLY) laterOn(() => stamp(geoGate, matViolet, 360, 780, Math.ceil(22), 14, 22, 29, 8, 12));
	if (!CORE_ONLY) laterOn(() => stamp(geoDen, matViolet, 480, 980, Math.ceil(40), 7, 13, 53, 6, 10));
	if (!CORE_ONLY) laterOn(() => stamp(geoHallBlock, matGold, 620, 1200, Math.ceil(24), 16, 28, 59, 8, 13));
	if (!CORE_ONLY) laterOn(() => stamp(geoNeedle, matCyan, 720, 1400, Math.ceil(30), 18, 40, 73, 5, 8));
	if (!CORE_ONLY) laterOn(() => stamp(geoDen, matGold, 900, 1680, Math.ceil(28), 7, 12, 81, 5.5, 9));
	if (!CORE_ONLY) laterOn(() => stamp(geoNeedle, matViolet, 1100, 1900, Math.ceil(18), 16, 34, 97, 4.5, 7));
	laterOn(() => {
		if (CORE_ONLY) {
	...
	const lampN = CORE_ONLY ? 0 : (coarse ? 90 : 200);
	const lampPal = [
		8319231,
		16765040,
		11571455,
	...
	const discs = [];
	if (!CORE_ONLY) DISTRICTS.forEach((D, i) => {
	...
	for (let i = 0; i < (CORE_ONLY ? 0 : 6); i++) {
		const a = i / 6 * Math.PI * 2;
		const disc = new Mesh(geoDiscBody, i % 2 ? matDiscB : matDiscA);
		disc.scale.set(1.05, .26, 1.05);
		disc.userData.tx = Math.cos(a) * 80;
		disc.userData.tz = Math.sin(a) * 80;
	...
	for (let i = 0; i < (CORE_ONLY ? 0 : 22); i++) {
	...
	const grassN = CORE_ONLY ? 0 : (coarse ? 160 : 280);
  ...
  function pieceMesh(p: BuildPiece) {
	...
	function tick(t, dt, cam, resonance) {
		...
		if (innerCore) {
			...
			const by = Number(innerCore.userData.baseY) || 2.6;
			innerCore.scale.y = by + Math.sin(t * 1.4) * (by * 0.05);
		}
