...
export function startEngine(canvas: HTMLCanvasElement, onHud: HudFn): EngineHandle {
	...
	const fogBase = cheapGL ? 0.006 : (mobile ? 0.009 : 0.011);
	scene.fog = new THREE.FogExp2(cheapGL ? 0x0c1220 : 0x080e18, fogBase);
	...
	if (Math.hypot(player.x, player.z) > 40) {
		player.x = 9;
		player.z = 16;
	...
	function loop(now) {
		...
		if (mode === "title") {
			titleYaw += raw * .12;
			const dist = 28;
			camera.position.set(Math.sin(titleYaw) * dist, 14, Math.cos(titleYaw) * dist);
			camera.lookAt(0, 7, 0);
	...
	try {
		(window as unknown as { __controlsTest?: { getYaw: () => number; getSpeed: () => number; getX?: () => number; getZ?: () => number; setKeys?: (codes: string[]) => void } }).__controlsTest = {
			getYaw: () => player.yaw,
			getSpeed: () => player.speed,
			getX: () => player.x,
			getZ: () => player.z,
