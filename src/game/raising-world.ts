...
function tag(obj: THREE.Object3D, id: string) {
  obj.traverse((o) => {
    o.userData.pick = id;
  });
...
function noHit(obj: THREE.Object3D) {
  obj.traverse((o) => {
    o.raycast = () => {};
  });
}
...
export function buildRaising(): RaisingWorld {
  ...
  const parent = new THREE.Mesh(
    ...
    new THREE.MeshBasicMaterial({ color: 0xb8f4ff, fog: false, toneMapped: false }),
  );
  parent.position.set(-310, 72, 18);
  group.add(parent);
  noHit(parent);
  const parentHalo = new THREE.Mesh(
  ...
  const howlPad = new THREE.Mesh(
    new THREE.TorusGeometry(3.2, 0.08, 6, 24),
    new THREE.MeshBasicMaterial({ color: 0x7ee8f2, transparent: true, opacity: 0.55, depthWrite: false }),
  );
  ...
  const densifyRoot = new THREE.Group();
  group.add(densifyRoot);
  const densifyBits: THREE.Group[] = [];
  for (let i = 0; i < 8; i++) {
    const a = (i / 8) * Math.PI * 2 + 0.2;
    const bit = new THREE.Group();
    const crystal = new THREE.Mesh(new THREE.OctahedronGeometry(0.42, 0), i % 2 ? matShard : matGold);
    crystal.position.y = 0.55;
    crystal.scale.set(0.55, 1.15, 0.55);
  ...
  function tick(t: number, dt: number, howl: number, woken: number) {
    ...
    howlPad.scale.setScalar(1 + howl * 0.12);
    (howlPad.material as THREE.MeshBasicMaterial).opacity = 0.4 + howl * 0.5;
    parent.rotation.y = t * 0.08;
    ...
    const pulse = 0.85 + Math.sin(t * 3.2) * 0.35;
    beacon.scale.setScalar(0.85 + Math.sin(t * 4) * 0.18);
    (beacon.material as THREE.MeshBasicMaterial).opacity = 0.55 + pulse * 0.35;
  }

  function densify(grown: number) {
    densifyBits.forEach((bit, i) => {
      const on = i < grown;
      bit.visible = on;
      bit.scale.setScalar(on ? 1 : 0.001);
    });
  }

  function mark(id: string | null) {
    markId = id === "hub" || !id ? "hub" : id;
  ...
  mark("hub");
  return { group, heart, keepers, tick, densify, mark, dispose };
}
