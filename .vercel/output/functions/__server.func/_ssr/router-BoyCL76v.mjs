...
import { $ as encode, $t as ATTR_OPERATION_ID, A as jwtVerify, At as email, C as serializeSignedCookie, Cn as BetterAuthError, D as decodeJwt, Dt as array, E as base64Url, En as defineErrorCodes, Et as any, Ft as optional, It as record, Kt as normalizePathname, Lt as string, Mt as looseObject, Nt as number, O as decodeProtectedHeader, Ot as boolean, Pt as object, Qt as ATTR_HOOK_TYPE, Rt as union, S as serializeCookie, Sn as APIError, Tn as BASE_ERROR_CODES, Ut as betterFetch, Xt as withSpan, Zt as ATTR_CONTEXT, _ as runWithRequestState, a as createAuthorizationURL, an as runWithAdapter, b as createRouter$1, bn as isProduction, c as createRateLimitKey, cn as initGetModelName, d as deprecate, dn as createRandomStringGenerator, dt as JWTExpired, en as import_src, f as createAuthEndpoint, fn as createLogger, g as hasRequestState, gn as env, h as defineRequestState, i as refreshAccessToken, in as queueAfterTransactionHook, jt as literal, k as importJWK, l as findInvalidTrustedProxies, ln as initGetFieldName, m as isAPIError, mn as shouldPublishLog, n as socialProviders, nn as getAuthTables, o as applyDefaultAccessTokenExpiry, on as runWithTransaction, p as createAuthMiddleware, pn as logger, r as validateAuthorizationCode, rn as getCurrentAdapter, s as isLoopbackHost, sn as getBetterAuthVersion, t as SocialProviderListEnum, tn as safeJSONParse, u as getIp, un as generateId, v as getCurrentAuthContext, w as filterOutputFields, wn as kAPIErrorHeaderSymbol, x as toResponse, xn as isTest, y as runWithEndpointContext, yn as isDevelopment } from "../_libs/@better-auth/core+[...].mjs";
...
//#region node_modules/.nitro/vite/services/ssr/assets/router-BoyCL76v.js
...
var _0002_relics_default = "create table if not exists relics (\n  id         text primary key,\n  href       text not null unique,\n  created_at timestamptz not null default now()\n);\ncreate index if not exists relics_created_at_idx on relics (created_at desc);\n";
var _0003_relic_stats_default = "alter table relics add column if not exists plays integer not null default 0;\nalter table relics add column if not exists last_land timestamptz;\n";
/**
...
async function createPgliteSql() {
	...
	const migrate = async () => {
		const migrations = /* #__PURE__ */ Object.assign({
			...
			"/migrations/0002_relics.sql": _0002_relics_default,
			"/migrations/0003_relic_stats.sql": _0003_relic_stats_default
		});
...
var styles_default = "/assets/styles-DsurAIDw.css";
var APP_NAME = "The Howling Crucible";
var CRITICAL = `
...
var Route$6 = createRootRoute({
	head: () => ({
		meta: [
			...
			{
				name: "description",
				content: "The Howling Crucible — Pack HQ. Land the Circuit, open the Hall of Artifacts, or fly the constellation."
			},
			{
				name: "agent-capabilities",
				content: "Open the Citadel hub. Land the Circuit. Open Hall of Artifacts. Toggle the constellation sky."
			}
		],
		links: [
			{
			...
			{
				rel: "stylesheet",
				href: "https://fonts.googleapis.com/css2?family=Outfit:wght@400;500;600&family=Syne:wght@500;600;700;800&display=swap"
			},
			{
				rel: "stylesheet",
				href: styles_default
...
var $$splitComponentImporter$1 = () => import("./routes-BoVHVS11.mjs").then((n) => n.t);
var Route$5 = createFileRoute("/")({
	ssr: false,
	component: lazyRouteComponent($$splitComponentImporter$1, "component")
});
var $$splitComponentImporter = () => import("./login-Ry6VTHFl.mjs");
...
function sanitizeRemix(raw, wish = "") {
	if (!raw || typeof raw !== "object") return null;
	const o = raw;
	const name = clip(o.name, 28);
	const line = clip(o.line, 90);
	if (name.length < 2) return null;
	const buildings = (Array.isArray(o.buildings) ? o.buildings : []).slice(0, 6).map((b) => {
	...
	while (buildings.length < 3) buildings.push({
		name: "Unnamed den",
		kind: "den",
		line: "Still raising."
	});
	const theme = FORGE_THEMES.includes(o.theme) ? o.theme : "crystal";
	const log = (Array.isArray(o.log) ? o.log : []).map((l) => clip(l, 88)).filter(Boolean).slice(0, 12);
	return {
		id: clip(o.id, 40).replace(/[^a-zA-Z0-9_-]/g, "") || `remix-${name.toLowerCase().replace(/[^a-z0-9]+/g, "").slice(0, 12)}-${Math.random().toString(36).slice(2, 6)}`,
		name,
		...
		wish: clip(o.wish, 240) || clip(wish, 240),
		at: Number(o.at) || Date.now()
	};
}
var FORBIDDEN_KEY_FIELDS = [
	"xai_api_key",
	"api_key",
	"grok_api_key",
	"openai_api_key",
...
function rejectApiKeyPayload(body) {
	if (!body || typeof body !== "object" || Array.isArray(body)) return null;
	const rec = body;
	for (const key of FORBIDDEN_KEY_FIELDS) if (key in rec && rec[key] != null && rec[key] !== "") return "API keys are not accepted. Connect your Grok Bot account.";
	return null;
}
var SYSTEM = `You remix Luminous Circuit (Boltverse Odyssey Year 0 toy citadel).
...
/** Remix writes are local — no bot session ownership without sign-in. */
async function rejectBotWrite(_request, _body) {
	return null;
}
async function handleForge(request) {
	if (request.method !== "POST") return json$1({ error: "POST only" }, 405);
	const apiKey = process.env.XAI_API_KEY;
	if (!apiKey) return json$1({ error: "The forge is cold. Turn on SpaceXAI APIs when you publish." }, 503);
	let body = {};
	try {
		body = await request.json();
	} catch {
		return json$1({ error: "Bad howl." }, 400);
	}
	const keyErr = rejectApiKeyPayload(body);
	if (keyErr) return json$1({ error: keyErr }, 400);
	const blocked = await rejectBotWrite(request, body);
	if (blocked) return blocked;
	const wish = String(body.wish ?? "").replace(/\s+/g, " ").trim().slice(0, 240);
	if (wish.length < 4) return json$1({ error: "Howl a little longer." }, 400);
	const ac = new AbortController();
	const kill = setTimeout(() => ac.abort(), 32e3);
	const upstream = await fetch("https://api.x.ai/v1/chat/completions", {
		...
		body: JSON.stringify({
			...
			messages: [{
				...
				role: "user",
				content: `Remix Luminous Circuit:\n${wish}`
			}]
		})
	}).catch(() => null);
	if (!upstream || !upstream.ok || !upstream.body) {
		clearTimeout(kill);
		return json$1({ error: "Grok could not strike the remix." }, 502);
	}
	...
	const stream = new ReadableStream({ async start(controller) {
		...
		try {
			...
			if (world) send({
				...
				t: "error",
				v: "The remix would not hold."
			});
		} catch {
			send({
				t: "error",
				v: "The star core went quiet."
			});
		} finally {
			clearTimeout(kill);
			controller.close();
...
function worldFromStream(text, wish) {
	const events = {
		wish,
		at: Date.now(),
		buildings: [],
	...
	for (const ln of text.split("\n")) {
		...
		try {
			...
			else if (ev.t === "building" && ev.v) events.buildings.push(ev.v);
			else if (ev.t === "log" && typeof ev.v === "string") events.log.push(ev.v);
		} catch {}
	}
	return sanitizeRemix(events, wish);
}
var Route$3 = createFileRoute("/api/forge")({ server: { handlers: { POST: ({ request }) => handleForge(request) } } });
function idFromHref(href) {
	let n = 0;
	for (let i = 0; i < href.length; i++) n = n * 33 + href.charCodeAt(i) >>> 0;
	return `forged-${n.toString(36)}`;
}
function relicName(href) {
	try {
		return (new URL(href).hostname.split(".")[0] || "relic").replace(/[-_]+/g, " ").trim().replace(/\b[a-z]/g, (c) => c.toUpperCase()).slice(0, 28) || "Bound Relic";
	} catch {
		return "Bound Relic";
	}
}
function relicHost(href) {
	try {
...
function parseGrokSeed(raw) {
	...
	const host = url.hostname.toLowerCase();
	if (!(host === "grok.me" || host.endsWith(".grok.me") || host === "grok-sandbox.com" || host.endsWith(".grok-sandbox.com"))) return null;
	return { href: `${url.origin}${url.pathname === "/" ? "" : url.pathname.replace(/\/$/, "")}` };
}
var FILE = join(process.cwd(), "data", "relics.json");
async function readRelicFile() {
	try {
...
async function writeRelicFile(rows) {
	await mkdir(join(process.cwd(), "data"), { recursive: true });
	await writeFile(FILE, JSON.stringify(rows, null, 2));
}
function mergeRelics(a, b) {
...
function asRelic(row) {
	return {
		id: String(row.id),
		href: String(row.href),
		plays: Number(row.plays ?? 0),
		created_at: iso(row.created_at) ?? void 0,
		last_land: iso(row.last_land)
	};
}
async function handleRelics(request) {
	try {
    ...
    `);
		...
		const fileRows = (await readRelicFile()).map((r) => asRelic(r));
		if (request.method === "GET") {
			...
			if (fileRows.length) for (const r of rows) try {
				await sql.query("insert into relics (id, href, plays, last_land) values ($1, $2, $3, $4) on conflict (href) do nothing", [
					r.id,
					r.href,
					r.plays ?? 0,
					iso(r.last_land)
		...
		if (body.play && body.id) {
			try {
				await sql.query("update relics set plays = plays + 1, last_land = now() where id = $1", [body.id]);
			} catch (err) {
			...
			if (next) {
				if (!found) {
					...
					next.last_land = (/* @__PURE__ */ new Date()).toISOString();
				}
				await writeRelicFile(mergeRelics(fileRows, [asRelic(next)]));
			}
		...
		await sql.query("insert into relics (id, href) values ($1, $2) on conflict (href) do nothing", [id, seed.href]);
		const bound = {
			id,
			href: seed.href,
			plays: 0,
...
var handle$1 = ({ request }) => handleRelics(request);
var Route$2 = createFileRoute("/api/relics")({ server: { handlers: {
	GET: handle$1,
	POST: handle$1
} } });
var handle = async ({ request }) => {
	const { handleSignaling } = await import("./signaling.server-CLZQK0OA.mjs");
...
var rootRouteChildren = {
	...
	ApiForgeRoute: Route$3.update({
		id: "/api/forge",
		path: "/api/forge",
		getParentRoute: () => Route$6
	}),
	ApiRelicsRoute: Route$2.update({
		id: "/api/relics",
		path: "/api/relics",
		getParentRoute: () => Route$6
	}),
...
function getRouter() {
	...
	return createRouter({
		routeTree,
		basepath: raw && raw !== "/" ? raw : void 0,
		defaultErrorComponent: AppErrorComponent
	});
}
//#endregion
export { getSql as a, GENERIC_OAUTH_ERROR_CODES as c, relicName as i, PACKAGE_VERSION as l, parseGrokSeed as n, GROK_PROVIDERS as o, relicHost as r, getBaseURL as s, router_exports as t };
